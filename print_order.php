		...
		<div class="graph-form agile_info_shadow" >
		 	<h3 class="w3_inner_tittle two">----BILL RECEIPT----</h3>
				<div class="form-group" id="printablediv">
					<?php
                        $oid=$_GET['pid'];
                        $select=mysql_query("SELECT * FROM `prod_ordered_list` WHERE `prod_ord_id`='$oid'");
                        $row=mysql_fetch_array($select);
                        $user_id=$row['usr_id'];    
                        $user_name=$row['usr_name'];
                        $user_address=$row['usr_address'];
                        $cat_name=$row['cat_name'];
                        $quantity=$row['prod_ord_quantity'];
                        $product_price=$row['prod_ord_price'];
                        $total_amt=$row['prod_ord_tot_amt'];
                    ?>
                    <p> User Name        : <?php echo $user_name ?></p> 
                    <p> User Address     : <?php echo $user_address ?></p>
                    <p> Product Category : <?php echo $cat_name ?></p> 
