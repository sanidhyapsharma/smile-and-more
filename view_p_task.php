...
if (isset($_GET['dei'])) 
    {
        $dei=$_GET['dei'];
        $qry_delete=mysql_query("UPDATE `event_book_list` SET  `status` ='Done' WHERE `event_id`='$dei'");
        if ($qry_delete) 
        {
            echo "<script>
                alert('Event has is done.');            
                window.location='view_p_task.php';     
            </script>";
        }
        else
        {
				...
				<div class="container-fluid">
					<div class="navbar-header">
						...
						<ul class="nav navbar-nav">
						    ...
						    <li class="active" ><a href="#.php">Assigned Tasks</a></li>
						</ul>
				</div>
			</nav>	
	      <!-- //breadcrumbs -->
...
<div class="inner_content_w3_agile_info two_in">
	<div class="forms-main_agileits">
        ...
        <h3 class="w3_inner_tittle two">Assigned Tasks</h3>
			...
			<tbody style="color:blue;background-color: #BFBF7A;">
				<tr>
					<?php
					    ...
					    $qry_select_event=mysql_query("SELECT * FROM `event_assignment` WHERE `emp_id` = '$e_id'");
					    while ( $row=mysql_fetch_array($qry_select_event)) 
						{
						$eid=$row['event_id'];
					    $qry_select=mysql_query("SELECT * FROM `event_book_list` WHERE `event_id` = '$eid'");
					...
					?>
								...
								<td>
									<?php
										...
										$qry_select_status=mysql_query("SELECT `status` FROM `event_book_list` WHERE `event_id`='$event_id'");
									...
									<?php
										...
										{
									?>
						                  <a class="btn btn-warning" href="view_p_task.php?dei=<?php echo $row['event_id'] ?>">DONE</a>
									<?php
										}
									?>
								</td>	
