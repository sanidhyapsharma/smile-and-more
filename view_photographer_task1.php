					...
					<div class="navbar-header">
						...
						<ul class="nav navbar-nav">
						    <li ><a href="admin.php">Home</a></li>
						     <li ><a href="#">Photographer Tasks</a></li>
						</ul>
				</div>
			</nav>	
	      <!-- //breadcrumbs -->
	...
	<div class="inner_content_w3_agile_info two_in">
	    <div class="forms-main_agileits">
			<div class="wthree_general graph-form agile_info_shadow " style="background: url('images/images(12).jpg');background-size:cover; width:80%; text-align:center;">
             	<h3 class="w3_inner_tittle two">Photographer Task Details</h3>
...
// code to assign the studio manager
	if (isset($_POST['btn_view_p'])) 
	{
		
		$emid=$_POST['emid'];
		                                        
                           $qry_category=mysql_query("SELECT * FROM `event_assignment` where `emp_id`='$emid'");
                                while ($row=mysql_fetch_array($qry_category)) 
                                 {
                                           ...
                                           $qry_category1=mysql_query("SELECT * FROM `event_book_list` where `event_id`='$event_id'");
                                           while ($row1=mysql_fetch_array($qry_category1)) 
                                           {
                                           	  $event_type=$row1['event_type'];
                                           	  $start_date=$row1['strt_date'];
                                           	  $start_time=$row1['strt_time'];                                          
                                           	  $end_date=$row1['end_date'];
                                           	  $end_time=$row1['end_time'];
                                           	  $event_place=$row1['event_place']

                                ?>
								<form method="post" action="">
									<td style="color:red;"> <?php echo $event_type?></td>
                                    <td style="color:red;"> <?php echo $start_date?></td>
                                                                                         
                                    <td style="color:red;"> <?php echo $start_time?></td>

                                    <td style="color:red;"> <?php echo $end_date ?></td>

                                    <td style="color:red;"> <?php echo $end_time ?> </td>
									
									  
									  <td style="color:red;"> <?php echo $event_place ?> </td>
                                </form>
                            </tr>
                                <?php
                                		}
