...
// code to assign the studio manager
	if (isset($_POST['btn_assign'])) 
	{
		$eid=$_POST['eid'];
		$pid=$_POST['pid'];
		$qry_assignment=mysql_query("INSERT INTO `prod_assignment`(`assign_id`, `prod_ord_id`, `emp_id`) VALUES ('','$pid','$eid')");

		if ($qry_assignment) 
		{
			$qry_update_status=mysql_query("UPDATE `prod_ordered_list` SET `prod_status`='Assigned' WHERE `prod_ord_id`='$pid'");
			if ($qry_update_status) 
			{
				echo "<script>
								alert('Studio Manager Assigned Successfully!..');
...
<!-- code for assign studio manager -->
<div class="inner_content_w3_agile_info two_in">
	<div class="forms-main_agileits" >
		<div class="wthree_general graph-form agile_info_shadow " style="background: url('images/images(13).jpg');background-size:cover; width:70%; text-align:center;">
	        <h3 class="w3_inner_tittle two">Assign Studio Manager</h3>
				<div class="grid-1 ">
					<div class="form-body">
						<form class="form-horizontal" method="post" action="assign_studio.php" enctype="multipart/form-data">
							<div class="form-group">
								<label for="focusedinput" class="col-sm-2 control-label"></label>
                            		<input type="hidden" name="pid" value="<?php echo $_GET['pid'] ?>">
										<div class="col-sm-6">
											<select id="category" class="form-control" name="eid" required="">
												<option value="">Select Studio Manager</option>
													<?php
														$qry_category=mysql_query("SELECT * FROM `emp_details` where `emp_type`='studiomanager'");
	                        ...
	                        <div class="form-group">
								<label for="focusedinput" class="col-sm-2 control-label"></label>
								<div class="col-md-8">
									<button type="submit" class="btn btn-default" name="btn_assign">Assign Studio Manager</button>
							</div>
						</div>
					</form>
			</div>		
